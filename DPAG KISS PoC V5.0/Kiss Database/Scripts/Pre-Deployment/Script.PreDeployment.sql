...
IF NOT EXISTS (SELECT * FROM [sys].[server_principals] WHERE name = N'$(KissProcessingLogin)')
	CREATE LOGIN [$(KissProcessingLogin)] 
		WITH PASSWORD = 'pass@word1', DEFAULT_DATABASE=[master], DEFAULT_LANGUAGE=[us_english]

GO
		
IF NOT EXISTS (SELECT * FROM [sys].[database_principals] WHERE name = N'$(KissProcessingLogin)')
	CREATE USER [$(KissProcessingLogin)] 
		FOR LOGIN [$(KissProcessingLogin)] 
		WITH DEFAULT_SCHEMA=[dbo]
